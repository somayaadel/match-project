...
<template>
  <div class="container" v-if="userData.user_data" :key="key">
    ...
    <div class="vx-row mb-6">
      ...
      <div class="vx-col sm:w-2/3 w-full">
        <vs-input
          type="number"
          class="w-full"
          v-model="userData.user_data.phone"
        />
      </div>
    </div>
    ...
    <div class="vx-row mb-6">
      ...
      <div class="vx-col sm:w-2/3 w-full">
        <datepicker
          ...
          v-model="userData.user_data.birth_date"
        ></datepicker>
      </div>
    </div>
    ...
    <div class="vx-row mb-6">
      ...
      <div class="vx-col sm:w-2/3 w-full">
        <vs-input
          type="text"
          class="w-full"
          v-model="userData.user_data.address"
        />
      </div>
    </div>
    ...
    <div class="vx-row mb-6">
      ...
      <div class="vx-col sm:w-2/3 w-full">
        <vs-select
          name="gender"
          v-validate="'required'"
          v-model="userData.user_data.gender"
    ...
    <div class="vx-row mb-6">
      ...
      <div class="vx-col sm:w-2/3 w-full">
        <vs-select
          name="service_id"
          v-validate="'required'"
          v-model="userData.user_data.package_id"
    ...
    <div class="vx-row mb-6" v-for="(item, index) in fields" :key="index">
      ...
      >
        <vs-input
          :type="item.type"
          class="w-full"
          v-model="userData.fields[item.name]"
        />
      </div>
      <div
        class="vx-col sm:w-2/3 w-full"
    ...
    <div class="logo">
      <img
        :src="
          userData.user_data.profile_image
            ? userData.user_data.profile_image
  ...
  methods: {
    ...
    getUserData() {
      axios.get("/api/user/" + this.$route.params.id).then((res) => {
        ...
        this.checkForUserOnApplication() ; 

        this.userData.categories.map((categ) => {
          if (categ.category_type_id == 1) {
            this.interestsData.push(categ.id);
          } else if (categ.category_type_id == 2) {
            this.actorsData.push(categ.id);
          }
        });
        console.log("actor", actorsData);
        this.packageChanged(this.userData.user_data.package);
      });
    },
    ...
    sendUserPutRequest() {
      ...
      axios
        .put("/api/user/" + this.$route.params.id, this.userData)
        .then((res) => {
          this.showSuccess("user has been updated successfuly");
